// Real browser image picker → async decode → canvas → SQLite/OPFS → reload.
// After `day build -p web-dom`:
// DAY_WEB_DRIVER_PLAYWRIGHT=<dir containing node_modules/playwright> node scripts/web-image-check.mjs [dist] [chromium|webkit]
...
const profile = fs.mkdtempSync(path.join(os.tmpdir(), 'day-sketch-images-'));
let context;
try {
  ...
  await page.locator('.day-toolbar-menu-item').filter({ hasText: 'Image…' }).click();
  await (await chooser).setFiles('resource/images/app_logo.png');
  ...
  fs.mkdirSync('build/day/screenshots/web-images', { recursive: true });
  await page.screenshot({ path: 'build/day/screenshots/web-images/inserted.png' });
  ...
  await page.screenshot({ path: 'build/day/screenshots/web-images/reopened.png' });
  assert.deepEqual(errors, []);
  console.log(`${engine}: image picker, async canvas decode, edits and OPFS reload passed`);
} finally {
